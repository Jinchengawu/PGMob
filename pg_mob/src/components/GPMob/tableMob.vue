     ...
     @selection-change="handleSelectionChange">
      ...
      >
        <template slot-scope="scope">
          <span @[MO[item.key]]="_tabEvns(item,scope.row)" :class="'v-'+item['key']">
            {{ scope.row[item['key']] }}
            <slot :name="item.key" v-if="tabsSlots[item.key]">定制化form</slot>
                <!-- <slot v-if="tabsSlots[item.key]">
                     <template>
                        <el-button  type="text">{{$t('update')}}</el-button>
                        <el-button @click="integralThis(scope.row)" type="text">{{$t('look')}}</el-button>
      ...
      <!-- <el-table-column 
        v-for='(item,index) in tableConfig.operating'
        :key ='index'
        label="操作"       
        width="55">
        <template slot-scope="scope">          
           <span  @[MO[item.key]]="_tabEvns(item,scope.row)" :class="'v-'+item['key']" >{{ scope.row[item['key']] }}
            <slot :name='item.key' v-if="tabsSlots[item.key]">定制化form</slot>
          </span>
        </template>
      </el-table-column>         -->
